...
def get_next_action(current_row_index, current_column_index, epsilon):
  ...
  if np.random.random() < epsilon:
    return np.argmax(q_values[current_row_index, current_column_index]) # this is where the past board state is taken and the best output for that state is choosen if epsilon is less than whatever it is defined as????
